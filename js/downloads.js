document.addEventListener("DOMContentLoaded", () => {
        ...
        btn.addEventListener('click', () => {
            ...
            stepSections.forEach(item => {                
                if (item.dataset.step === currentStep) {                    
                    ...
                    item.classList.remove('section-hidden')
                        if (item.classList.contains(`step__section--${added}`)) {
                            item.classList.remove('section-hidden');                                
                        } else {
    ...
    inputAudio.addEventListener('change', function(event) {    
        ...
        if (event.target.files && event.target.files[0]) {
            ...
            document.querySelector('.added-singl__name').textContent = `Название трека: ${newTrack.title || "Не известно"}`;
            document.querySelector('.added-singl__img').src = imgPath || NO_FOTO;
            document.querySelector('.added-singl__autor').textContent = `Исполнитель: ${newTrack.artist || "Не известен"}`;
    ...
    formFive.addEventListener('submit', (event) => {
        ...
        newTrack.important_info = {
            linkSingerOne: document.getElementById('link-1').value, //Ссылка на карточку исполнителя в ВК
            linkSingerTwo: document.getElementById('link-2').value, //Ссылка на карточку исполнителя в Тик-Ток
            linkSingerThree: document.getElementById('link-3').value,
        } //Ссылка на карточку исполнителя в Яндекс Музыка
        console.log('Новый трек шаг 5', newTrack);  

        // Добавляем отображение названия и обложки с шага 2
        document.querySelector('.payment__name').textContent = `Название трека: ${newTrack.title || "Не известно"}`;
        document.querySelector('.payment__img').src = imgPath || NO_FOTO;
        document.querySelector('.payment__autor').textContent = `Исполнитель: ${newTrack.artist || "Не известен"}`;       
    });
